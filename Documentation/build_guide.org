...
* V1.1 Changes
- Sacrificial layers for square nuts gone for blaster_grip, can ignore directions saying to drill those.
- New nub catch design.
- Mag release handles worker 15 dart mags.
- Mag release has arrow to indicate direction (up towards blaster top). 
...
- M3 screws, hex socket cap. < 3mm head height, < 6mm head diameter.
  ...
  - a M3 screw with 30mm of smooth shaft followed with at least a few mm of threads. I cut down a 55mm M3 screw with a smooth section for this. This is for the nub catch (v1.1 change). 
...
Slide the trigger into the groove in the grip.
...
** Nub Catch
This pushes back the ball retention nub to release the balls from the magazine. 
V1.1 uses an M3 screw instead of all plastic.
This is the M3 screw with 30mm of smooth section followed by a few mm of threads (I had 10 mm of threads, so M3x40 effectively, with 30mm smooth).
I used a cut down 55mm M3 for this.
Threaded shaft works, but while the smooth screw is super smooth, threads caused some bumpiness, so I advise finding a screw with a smooth, unthreaded section.

Anyway, just screw the screw all the way into the hole shown. 
[[./documentation_images/physical_build/used_images/new_catch_side_annotated.jpg]]
[[./documentation_images/physical_build/used_images/new_catch.jpg]]

This will avoid the below happening.
...
- Place the  catch into the magazine body.
  - Catch orientation is very important here! There is an arrow on the catch, on the side facing the magazine. This arrow should be facing up, towards the blaster top. Note that the images shown here are of an older catch without the arrow.
  #+attr_html: :width 400px
...
[[./documentation_images/physical_build/used_images/battery_inserted.jpg]]

This is designed for a 1000 mah 4s "graphene" pack.

Some other packs will work, there's an 850 mah 4s nanotech that is also suitable.

To insert the battery, I place the balance plug to the side, under the side railing that the top rail/battery cover rests on.
This can be tricky, as the battery + balance plug is too thick to drop in, so must be angled in, or first place the battery in then snake the balance plug.

Regardless, one this is done, the XT60 can rest against the FWC covers as shown, with all the wires pushes to one side allowing the XT60 to sit directly on plastic, which gives the battery cover enough room to close.

The 850 mah battery is quite a bit easier to insert 😉.
